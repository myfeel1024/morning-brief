...
def _newsapi_search(query: str, since_iso: str, sort_by: str, page_size: int,
    ...
    try:
        ...
        for a in data.get("articles", []):
            title = a.get("title", "")
            if not title or any(kw in title.lower() for kw in _NEWS_SKIP_KEYWORDS):
                continue
            source = a.get("source", {}).get("name", "")
            results.append(f"{title} ({source})")
        return results
...
② 핵심 매크로 포인트
   ...
   - 달러/원 환율 영향
   - 유가·원자재 이슈 (해당 시)
③ 주요 기업 실적 — 아래 [기업 실적 발표 뉴스]만 근거로 작성. '실제로 발표된 실적'과 '아직 발표 전인 전망'을 반드시 구분하세요.
   ※ 작성 규칙 (반드시 준수):
   - [1단계] '실제로 실적이 이미 발표·공시된' 기업만 판정하세요. 기사에 결과가 확정됐다는 과거형 표현(발표했다/공시/reported/posted/beat/missed/results 등)이 있을 때만 다음 중 하나로 분류하고 뒤에 '(발표됨)'을 붙이세요: "어닝 서프라이즈"(예상 상회) / "어닝 쇼크"(예상 하회) / "시장 예상 부합".
   - [필수] 실제로 발표된 실적이 하나도 없으면 맨 앞에 "실제 발표된 실적: 없음"이라고 명확히 적으세요. 절대 전망을 발표된 실적처럼 쓰지 마세요.
   - [2단계] 아직 발표 전인데 전망·예상·프리뷰성 기사(예: "예상 상회 전망", "전망 보도", "expected to beat", "ahead of earnings", "preview")는 확정처럼 쓰면 안 됩니다. 반드시 "[전망] OO → 어닝 서프라이즈 예상됨 (발표 전)" 처럼 '예상/전망이며 발표 전'임을 표기하세요. 예: "LG이노텍 → 어닝 서프라이즈 예상됨 (발표 전, 확정 아님)".
   - 결과인지 전망인지 헤드라인만으로 애매하면 '전망'으로 취급하고 [전망]으로 표기하세요. 주가 반응으로 실적을 추측하지 마세요.
   - 주가 등락(급락/급등 %)은 언급하지 마세요.
   - 같은 기업에 대해 시점이 다른 기사가 여러 개 있으면, 발표 시각이 가장 최신인 기사 내용을 우선해서 반영하세요.
   - 발표된 실적도 전망 기사도 전혀 없으면 "특이 실적 발표 없음"이라고만 짧게 적기.
